                        ...
                        <div class="box box-solid">
                            ...
                            <div class="box-body no-padding">
                                <ul class="nav nav-pills nav-stacked">
                                    ...
                                    @endphp
                                    <li class="{{$tipe == 'Masuk' ? 'active' : ''}}"><a
                                            href="{{route('surat.index')}}?tipe=Masuk"><i class="fa fa-inbox"></i> Surat
                                            Masuk
                                            <span class="label label-primary pull-right">12</span></a></li>
                                    <li class="{{$tipe == 'Keluar' ? 'active' : ''}}"><a
                                            href="{{route('surat.index')}}?tipe=Keluar"><i class="fa fa-envelope-o"></i>
                                            Surat Keluar</a></li>
                                </ul>
                            </div>
                        </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{route("$template->route".'.edit',[$row->id])}}"
                                                    class="btn btn-success btn-sm {{AppHelper::config($config,'index.edit.is_show') ? '' : 'hidden'}}">Ubah</a>
                                                <a href="{{route("$template->route".'.show',[$row->id])}}"
                                                    class="btn btn-info btn-sm {{AppHelper::config($config,'index.show.is_show') ? '' : 'hidden'}}">Lihat</a>
                                                <a href="#"
                                                    class="btn btn-danger btn-sm {{AppHelper::config($config,'index.delete.is_show') ? '' : 'hidden'}}"
                                                    onclick="confirm('Lanjutkan ?') ? $('#frmDelete{{$row->id}}').submit() : ''">Hapus</a>
                                                <form action="{{route("$template->route".'.destroy',[$row->id])}}"
                                                    method="POST" id="frmDelete{{$row->id}}">
                                                    {{ csrf_field() }}
                                                    @method('DELETE')
                                                </form>
                                            </td>
